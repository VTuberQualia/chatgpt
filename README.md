...
依存関係は `requirements.txt` にまとめています。次のコマンドでインストールしてください。
...
```bash
python -m kickbike_analysis.train data --epochs 10 --output model.pt
```

推論は下記のように行います。

```bash
python -m kickbike_analysis.infer video.mp4 --weights model.pt
```
